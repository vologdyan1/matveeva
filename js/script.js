...
const init = () => {
  ...
  let cachedHeaderHeight = parseInt(getComputedStyle(document.documentElement).getPropertyValue("--header-height") || "80", 10);
  const updateHeaderHeight = () => {
    cachedHeaderHeight = header?.offsetHeight ?? cachedHeaderHeight;
  };
  window.addEventListener("resize", updateHeaderHeight, { passive: true });
  updateHeaderHeight();
  const HEADER_OFFSET_PX = () => cachedHeaderHeight;
    ...
    const initCarouselUI = () => {
      ...
      if (viewport) {
        ...
        viewport.addEventListener("touchend", (e) => {
          ...
          if (dx < -SWIPE_THRESHOLD) setSlide(currentIndex + 1);
          else if (dx > SWIPE_THRESHOLD) setSlide(currentIndex - 1);
          touchStartX = 0;
          isSwiping = false;
        }, { passive: true });
      }
